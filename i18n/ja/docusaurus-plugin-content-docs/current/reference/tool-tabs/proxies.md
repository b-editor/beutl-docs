...
**プロキシメディアを生成・管理する**タブです。プロキシとは重い動画クリップの低解像度コピーで、プレビューやスクラブを軽快にします。編集中のプレビューはプロキシを使用できますが、**書き出しは常に元のメディアを使用します**。
...
- プロキシはこのタブから**手動で**生成します。読み込み時に自動生成されることはありません。
...
- プロキシはソースファイルの**パス・サイズ・更新日時**をキーに管理されます。いずれかが変わるとプロキシは**期限切れ**になり、再生成するまで未生成として扱われます。同じファイルを参照する複数のクリップ(および複数のプロジェクト)は1つのプロキシを共有します。
- プロキシは**マシン全体で共有されるストア**(既定: Beutlのホームディレクトリ内の `proxies` フォルダー)に保存されます。ストアがサイズ上限を超えると、最も使われていないプロキシから自動的に削除されます。ストアの設定は[エディタ設定](../../settings/editor.md#プロキシメディア)を参照してください。

## タブの構成

- **ストアの使用状況(上部)**: プロジェクト / ストア / 上限の使用量。
- **クリップ一覧**: シーン内の対象クリップごとに以下を表示します。
...
**すべて生成**は1920x1080以上(解像度が不明な場合は32 MB以上)のクリップのみを対象とします。軽いクリップはクリップごとの**生成**を使用してください。対象がない場合は「一括生成のしきい値を満たすクリップがありません」という通知が表示されます。

:::warning
プロキシストアはマシン全体で共有されます。**プロジェクトのプロキシを削除**は、同じソースファイルを参照する他のプロジェクトが使っているプロキシファイルも削除します。実行前に確認ダイアログで警告されます。
:::

## エディタ上でのプロキシ

- タイムラインでは、プロキシ状態のあるクリップの右上に小さな**色付きドット**が表示されます(準備完了 / 生成中 / 期限切れ / 失敗)。ホバーすると詳細を確認できます。
- プレビューがプロキシを使うかどうかは **「プレビューソース」** オプションで制御します。**プロキシ優先**(既定)または**オリジナル強制**を、**設定 → エディタ → プロキシメディア** とエディタのプレビュー設定タブで切り替えられます。切り替えはプロジェクトを再読み込みせずに次のフレームから反映されます。
- プロキシが未生成・期限切れ・一部生成の場合、プレビューは自動的に元のメディアにフォールバックします。
- **書き出しはプロキシを使用しません。** 書き出し時に元のファイルが見つからない場合は、プロキシで代替せずエラーで失敗します。
