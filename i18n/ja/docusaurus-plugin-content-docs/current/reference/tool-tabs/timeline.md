...
何もない場所の右クリックメニューの **「編集モード」** サブメニュー（または上記のキーボードショートカット）で、排他的なツールモードを切り替えられます。モードが有効な間はタイムラインにモード名入りの色付き枠が表示されます。`V` または `Esc` で通常の選択ツールに戻ります。
...
**リップル編集** (`B`) はツールモードではなく持続的なオン/オフ設定で、オフにするまで有効なままになり、セッションをまたいで記憶されます。有効な間は、以下の操作の後に同じレイヤー上の後続クリップが詰められ（または空けられ）ます。

- **削除** / **除外** / **切り取り**
- 要素の端のトリム

ロックされたレイヤーは動かず、ロックされたクリップはアンカーとして扱われ、そこでリップルの伝播が止まります。

## 隙間を詰める・隙間へ移動する

同じレイヤー上のクリップ間の空白は **隙間** として扱われます。何もない場所の右クリックメニューの **「隙間」** サブメニュー、またはキーボードショートカットで操作します。

- **隙間を詰める**: 1つの隙間を詰めます。メニューからは右クリック位置の隙間、ショートカット (`;`) では選択した要素の後ろの隙間が対象です。
...
要素の右クリックメニューで **「ロック」** をトグルします。ロックされた要素は半透明+ロックアイコン付きで表示され、移動・トリム・分割・リネーム・削除ができなくなります。また、リップルや隙間の操作ではアンカーとして扱われます。

### レイヤーのトグル

各レイヤーヘッダーには4つのトグルボタンがあります。
...
- **ソロ**: いずれかのレイヤーがソロの間、ソロでないレイヤーは表示も再生もされません。複数のレイヤーを同時にソロにできます。
