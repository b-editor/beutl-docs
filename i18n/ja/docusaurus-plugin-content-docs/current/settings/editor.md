...
ここでは、[プロキシメディアワークフロー](../reference/tool-tabs/proxies.md)に関する設定を変更できます。

### プレビューソース

プレビューでプロキシメディアを使用するかどうかを設定します。
...
プロキシファイルは、このフォルダーに保存されます。

既定の保存先は、Beutlのホームディレクトリ内にある `proxies` フォルダーです。変更はBeutlの再起動後に反映されます。

### 最大ストアサイズ

ストアがこのサイズ (GiB) を超えると、Beutlが最も使われていないプロキシから削除します。

デフォルト値は __"50"__ GiB です (設定可能範囲: 5〜500)。変更はBeutlの再起動後に反映されます。

### 既定のプリセット

プロキシツールタブで最初に選択される品質プリセットを設定します。
